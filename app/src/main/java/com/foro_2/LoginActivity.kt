...
class LoginActivity : AppCompatActivity() {
    ...
    private val googleSignInLauncher = registerForActivityResult(
        ...
        if (result.data == null) {
            ...
            if (result.resultCode == android.app.Activity.RESULT_CANCELED) {
                ...
                Toast.makeText(this, "Error: No se recibieron datos de Google", Toast.LENGTH_SHORT)
                    .show()
                Log.e(
                    "LoginActivity", "result.data es null pero result code es ${result.resultCode}"
                )
            }
        ...
        if (task.isSuccessful) {
            try {
                ...
                if (idToken != null) {
                    ...
                    Toast.makeText(this, "Error: token de Google es nulo", Toast.LENGTH_SHORT)
                        .show()
                    Log.e(
                        "LoginActivity",
                        "idToken nulo en Google Sign-In. Account: ${account?.email}"
                    )
                }
            } catch (e: Exception) {
                Toast.makeText(
                    this, "Error al procesar cuenta de Google: ${e.message}", Toast.LENGTH_SHORT
                ).show()
                ...
                if (exception != null) {
                    val errorMessage = when (exception.statusCode) {
                        ...
                        12500 -> "Error al iniciar sesión. Intenta de nuevo"
                            ...
                            val msg =
                                "Google Sign-In falló (código ${exception.statusCode}): ${exception.message}"
                    ...
                    Log.e(
                        "LoginActivity",
                        "Status code: ${exception.statusCode}, Message: ${exception.message}"
                    )
                } else {
                    val errorMsg = "Error desconocido: ${task.exception?.message ?: "Sin detalles"}"
                    Toast.makeText(this, errorMsg, Toast.LENGTH_SHORT).show()
                    Log.e(
                        "LoginActivity", "Error en Google Sign-In (no ApiException)", task.exception
                    )
                }
            } catch (e: Exception) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            if (::googleSignInClient.isInitialized) {
                ...
                try {
                    ...
                    if (signInIntent != null) {
                        ...
                        Toast.makeText(
                            this,
                            "Error: No se pudo crear el intent de Google Sign-In",
                            Toast.LENGTH_SHORT
                        ).show()
        ...
        binding.facebookSignInButton.setOnClickListener {
            Toast.makeText(this, "Inicio de sesión con Facebook próximamente", Toast.LENGTH_SHORT)
                .show()
